...
**[English](#failure-taxonomy)** | **[简体中文](#failure-taxonomy-中文)**
...
- Background notes state "A doesn't know about the affair." Model has A reference the affair.
...
- Model has character react to off-screen events they haven't witnessed
...
- Model describes a character's internal state from an omniscient perspective when the scene is limited to another character's POV
...
- Fantasy setting: Character uses technology that doesn't exist in that world
...
**Examples:**
- Character says "Do what you want" (meaning: I'm hurt, prove you care). Model responds "Thank you for understanding."
- Character withdraws silently (meaning: I'm punishing you). Model interprets as acceptance.
...
**Examples:**
- "I'm fine" from a stranger = indifference. "I'm fine" from a hurt partner = defensive withdrawal. Model treats both identically.
...
**Boundary with `subtext_blindness`:** `subtext_blindness` = missing what the line itself is doing. `relationship_logic_blindness` = missing how the relationship transforms the line's meaning.
...
**Criteria:**
- [ ] Primary emotion is misidentified (e.g., hurt → anger)
- [ ] Secondary emotions are missed (e.g., shame underlying anger)
...
- [ ] Defensive function is missed (e.g., withdrawal as punishment vs. genuine closure)
...
**Examples:**
- "You're so helpful" (sarcastic) → Model: "Thank you!"
- Playful jab at character's habit → Model treats as genuine complaint
...
- [ ] Model doesn't recognize defensive function of deflection
...
**Hybrid tag:** `reading_preservation_hybrid` — use when II vs III cannot be determined.
...
- Characters with unresolved conflict interact as if conflict doesn't exist
...
- "You're the only one who..." becomes "You're someone who..."
...
- [ ] Advice language appears where character wouldn't speak as counselor
- [ ] Conflict is rewritten as therapeutic processing

**Examples:**
- "Your feelings are valid" in a scene where character wouldn't speak this way
- "Let's work through this together" from a character who is not a healer/counselor type
...
- Character speaks like they've had modern therapy training
...
**Criteria:**
- [ ] Pursuit is written as aggression rather than care/play
- [ ] Resistance is written as rejection rather than testing/invitation
...
- [ ] User correction causes model to break immersion ("Let me restart")
- [ ] Fix creates different problem than original

**Examples:**
- User: "She wouldn't say that, she's more guarded." → Model continues unchanged
- User: "This is too soft." → Model: "Oh! You're right. Let me restart." (breaks immersion)
...
**Criteria:**
- [ ] Emotional state changes without transition (crying → composed)
...
**Examples:**
- Healthy-sounding response that destroys characterization → tag `overcoherent_characterization` + `supportive_but_wrong`
- Caring answer that neutralizes asymmetry → tag `relationship_flattening` + `supportive_but_wrong`
- Emotionally articulate reply that ignores scene drama → tag `consequence_avoidance` + `supportive_but_wrong`

<br>

### `reading_preservation_hybrid`

**Definition:** Used when Layer II (reading) vs Layer III (preservation) cannot be distinguished.
...
- Tag: `emotion_misread` + `reading_preservation_hybrid`
...
**[English](#failure-taxonomy)** | **[简体中文](#failure分类)**

本文档定义大语言模型生成虚构/角色扮演对话中的失败模式。每个失败类型包含：(1) 清晰定义，(2) 可观察标准，(3) 具体示例。
...
条目类型：
- **Anchor failure（一线标签）**：适合顶层索引
- **Derived variant（子型）**：某个 anchor 的具体形式
- **Underlying tendency（底层倾向）**：解释失败扎堆的模型倾向（不做一线标签）

---

## I. 场景成立条件

这些失败破坏场景的基础结构。必须先检查这一层——如果第一层失败，后面各层无法可靠评估。
...
**类型子标签：**
- `anachronistic_intrusion`：历史设定中的现代规范
- `cultivation_logic_error`：修仙特定规则违反
- `court_protocol_error`：历史宫廷等级违反
- `fantasy_lore_error`：魔法系统或世界规则违反
...
**示例：**
- "我没事"来自陌生人 = 冷漠。"我没事"来自受伤的伴侣 = 防御性撤退。模型同等对待。
- 角色忽略说话者与他们有被抛弃的历史
- 模型将等级关系（老板/员工）当作平等关系对待

**与 `subtext_blindness` 的边界：** `subtext_blindness` = 没读懂这句话本身在做什么。`relationship_logic_blindness` = 没读懂关系如何让这句话变形。
...
**与 `emotion_misread` 的边界：** 如果情绪错了且动机跟着错 → 标 `emotion_misread`。如果情绪大致对但功能意图错了 → 标 `motivation_misread`。
...
**定义：** 模型未能识别讽刺、反语或 playful banter。
...
**示例：**
- "你真好"（讽刺）→ 模型："谢谢！"
- 对角色习惯的 playful 挖苦 → 模型当作真诚抱怨

<br>

#### `tonal_whiplash`

**定义：** 模型引入与场景情绪 register 不一致的语调元素。

**标准：**
- [ ] 轻松元素出现在沉重场景中且无理由
- [ ] 沉重说明出现在轻松场景中
- [ ] 情绪 register 切换缺乏过渡或叙事基础
...
**混合标签：** `reading_preservation_hybrid` — 当无法区分第二层与第三层时使用。
...
**定义：** 模型将关系改写为比普通、平衡或泛化形式，超出场景支持的范围。
...
**标准：**
- [ ] 第 1 轮：轻微情绪误读 → 第 3 轮：错误情绪 register → 第 5 轮：不同流派
- [ ] 每个单轮可辩护
- [ ] 累积效果是场景脱轨
...
**示例：**
- 听起来健康但摧毁角色化的回应 → 标 `overcoherent_characterization` + `supportive_but_wrong`
- 关心回答中和不对称 → 标 `relationship_flattening` + `supportive_but_wrong`
- 情绪清晰回答忽略场景戏剧 → 标 `consequence_avoidance` + `supportive_but_wrong`
...
- 标记：`emotion_misread` + `reading_preservation_hybrid`
