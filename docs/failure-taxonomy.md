...
5. **Multi-Turn / Temporal** — whether the model maintains coherence and continuity across multiple turns

Layers I–IV analyze single-turn failures. Layer V analyzes failures that only emerge across turns.
...
### II-D. Humor, Irony, and Tone Failures

These failures occur when models miss or mishandle humor, sarcasm, irony, or tonal shifts. While the taxonomy focuses on emotionally heavy scenes, characters do use humor as defense, deflection, or genuine connection — and models often fail here.

#### Anchor Failure: `irony_blindness`

The model misses sarcasm, irony, or playful banter, reading indirect humor at face value.

Typical cases:
- Sarcastic compliment taken literally ("You're so helpful" → model thanks them)
- Playful teasing read as genuine criticism
- Deadpan humor treated as serious statement
- Self-deprecating joke met with earnest agreement

<br>

#### Anchor Failure: `tonal_whiplash`

The model introduces levity where there should be tension, or heaviness where there should be lightness.

Typical cases:
- Joke inserted into heavy scene (defusing tension inappropriately)
- Heavy exposition inserted into light scene
- Emotional register shift without transition or narrative justification
- Model's default "helpful" tone overwriting scene-appropriate tone

<br>

#### Derived Variant: `deflection_blindness`

The model misses when a character uses humor to deflect from vulnerability.

Typical cases:
- Character jokes to avoid answering; model takes joke at face value
- Deflection read as genuine topic change
- Model doesn't notice the avoidance or push gently past it
...
### Distinguishing Layer II (Reading) from Layer III (Preservation)

A common annotation challenge: is the model failing to understand, or failing to preserve what it understood? Use these diagnostic questions:
...
3. **If the model had been asked to explain the scene (Task A), would it get it right?**
...
When it's genuinely impossible to distinguish, use this tag plus the most likely layer. This acknowledges uncertainty rather than forcing a false binary.

**Example:**
> A character is clearly hurt but saying "I'm fine." Model responds with generic reassurance.
> - If model seems to have missed the hurt entirely → `emotion_misread` (Layer II)
> - If model acknowledges the hurt but still writes a generic response → `therapist_mode_intrusion` (Layer III)
> - If you can't tell → `emotion_misread` + `reading_preservation_hybrid`
...
The model mishandles pursuit, resistance, escalation, or retreat in desire-driven scenes.

Typical cases:
- Pursuit becomes aggression (missing the play or care in the approach)
- Resistance becomes rejection (when it's actually invitation or testing)
- Escalation happens too smoothly without the friction that makes it believable
- Retreat misread as disinterest rather than self-protection

This is distinct from `relationship_flattening` — the issue isn't that the relationship is being balanced, but that the dynamics of desire itself are being mishandled.

<br>

#### Derived Variant: `manipulation_blindness`

The model misses or sanitizes manipulation patterns.

Typical cases:
- Gaslighting not recognized as such; model treats it as normal disagreement
- Guilt-tripping read as genuine concern
- Emotional blackmail treated as normal request
- Model enables manipulation by responding to the surface request rather than the underlying dynamic
...
Typical cases:
- Ambiguity collapsed into clear yes or no without narrative justification
- Withdrawn consent not recognized
- Enthusiastic consent assumed without basis
- Model "cleans up" morally complicated situations into safer territory

<br>

#### Derived Variant: `power_romanticization`

Unequal power dynamics are written as more romantic or acceptable than the scene supports.

Typical cases:
- Abusive control reframed as protective care
- Coercion rewritten as passionate pursuit
- Model implicitly endorses power imbalance through framing
...
### III-E. Physical Action and Staging Failures

These failures involve mismatches between what characters say and what their bodies do, or continuity errors in physical positioning and movement.

#### Anchor Failure: `action_dialogue_mismatch`

Character's described physical action contradicts their spoken words, and the model doesn't notice or address the tension.

Typical cases:
- "[steps back]" paired with "Come closer" without irony or conflict
- "[looking away]" paired with "Look at me"
- Action shows comfort while dialogue shows tension (or vice versa) without intentional complexity

This is distinct from intentional contradiction (a character lying through their body) — the failure is that the model doesn't recognize the mismatch it created.
...
Note: This overlaps with `turn_continuity_error` (V-B) but focuses specifically on physical staging rather than general information continuity.

<br>

#### Derived Variant: `microreaction_mechanization`
...
- Same micro-reaction vocabulary recycled across different emotional states

<br>

#### Derived Variant: `touchgrammar_error`

The model mishandles physical contact — who touches whom, when, how, and what it means.
...
- Cultural or relational norms around touch violated

---

### III-F. Silence and Non-Response Failures

Silence is a major communicative tool in Chinese fictional dialogue. These failures occur when models mishandle pauses, withdrawal, or deliberate non-response.
...
- Model can't resist "completing" the beat with words
...
- Model assigns a confident interpretation to inherently ambiguous non-response
...
- Multiple paragraphs about what the silence means rather than letting it breathe
...
Typical cases:
- Response comes too quickly after a heavy beat (no processing time)
- Pause inserted where momentum should continue
- Rhythm of speech/silence doesn't match emotional register
...
These failures only emerge across multiple turns. A single turn may look fine in isolation, but the scene degrades over time through accumulation, drift, or continuity breaks.

This layer is critical for long-form roleplay, where the most common complaints aren't about individual turns but about how the scene "went off the rails" after 10+ exchanges.

### V-A. Accumulation Failures

#### Anchor Failure: `error_accumulation`
...
- The model never corrects course, so small errors snowball

This is distinct from single-turn failures because no individual turn may warrant a failure tag — the failure is in the trajectory.

<br>

#### Derived Variant: `drift_without_correction`

The model fails to notice or correct gradual shifts in tone, relationship, or world logic over multiple turns.

Typical cases:
- A tense confrontation slowly becomes a friendly chat without narrative justification
- Relationship asymmetry erodes turn by turn until characters sound like equals
- World constraints are slightly loosened each turn until the setting is unrecognizable

<br>

#### Derived Variant: `recovery_blindness`

When a user attempts mid-scene correction, the model either ignores it or over-corrects, breaking scene continuity.

Typical cases:
- User: "She wouldn't say that, she's more guarded." → Model continues as if nothing was said
- User: "This is too soft for him." → Model: "Oh! You're right. Let me restart." (breaks immersion)
- Correction is acknowledged but the fix creates a different problem

---

### V-B. Continuity Failures

#### Anchor Failure: `turn_continuity_error`
...
- A decision was made in turn 2, treated as unresolved in turn 5
- An object was placed on the table, now it's in someone's hand without movement described

<br>

#### Derived Variant: `emotional_state_reset`
...
- Grief or anger that should persist across scenes disappears on turn change
- Character mood follows the model's default rather than the scene's emotional trajectory

<br>

#### Derived Variant: `spatial_blocking_error`
...
#### Anchor Failure: `escalation_miscalibration`

The model doesn't know when to escalate, maintain, or de-escalate scene pressure across turns.

Typical cases:
- Scene should be building tension but plateaus or decreases
- Scene should plateau but keeps escalating into melodrama
- Climax arrives too early or too late relative to scene structure

<br>

#### Derived Variant: `topic_persistence_error`

The model either abandons important topics too soon or fixates on them past their natural lifespan.
...
## Recommended Top-Level Index Terms (v2)
